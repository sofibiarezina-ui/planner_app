...
def render_mini_calendar(selected_date: date, user_data: dict) -> str:
    ...
    # Получаем сетку месяца: [[0,0,1,2,3,4,5], [6,7,...], ...]
    cal = calendar.monthcalendar(year, month)

    settings = user_data.get("settings", {})
    max_minutes = app.get_max_work_minutes(settings)
    important_dates = set(user_data.get("important_dates", []))
    tasks = user_data.get("tasks", [])

    # Простая CSS-стилизация под тему Streamlit
    html = """
    <style>
    ...
    for week in cal:
        ...
        for day in week:
            if day == 0:
                ...
                used_min = app.get_daily_load_minutes(tasks, date_str)
